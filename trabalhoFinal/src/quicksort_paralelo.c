#include <stdio.h>
#include <stdlib.h>
#include <omp.h>
#include "libppc.h"

static void swap(int *a, int *b) {
    int tmp = *a;
    *a = *b;
    *b = tmp;
}

static int partition(int *v, int low, int high) {
    int pivot = v[high];
    int i = low - 1;

    for (int j = low; j < high; j++) {
        if (v[j] <= pivot) {
            i++;
            swap(&v[i], &v[j]);
        }
    }
    swap(&v[i + 1], &v[high]);
    return i + 1;
}

static void quicksort_parallel_rec(int *v, int low, int high, int depth) {
    if (low < high) {
        int p = partition(v, low, high);

        // Após o particionamento, os subarrays [low, p-1] e [p+1, high]
        // não possuem elementos em comum.
        // Não há dependência de dados entre esses dois intervalos, então
        // podem ser executados em paralelo.

        if (depth <= 0) {
            // A partir de certa profundidade, executa de forma serial
            if (low < p - 1) quicksort_parallel_rec(v, low, p - 1, depth - 1);
            if (p + 1 < high) quicksort_parallel_rec(v, p + 1, high, depth - 1);
        } else {
            #pragma omp task shared(v)
            quicksort_parallel_rec(v, low, p - 1, depth - 1);

            #pragma omp task shared(v)
            quicksort_parallel_rec(v, p + 1, high, depth - 1);
        }
    }
}

void quicksort_parallel_impl(int *v, long int n, int num_threads) {
    int max_depth = 4; // limite para criar tasks

    #pragma omp parallel num_threads(num_threads)
    {
        // Apenas uma thread dispara as tasks iniciais
        #pragma omp single nowait
        {
            quicksort_parallel_rec(v, 0, (int)n - 1, max_depth);
        }
    }
}

int main(int argc, char *argv[]) {
    // ./quicksort_paralelo <N> <arquivo_entrada> <arquivo_saida> [num_threads]
    if (argc < 4 || argc > 5) {
        fprintf(stderr, "Uso: %s <N> <arquivo_entrada> <arquivo_saida> [num_threads]\n", argv[0]);
        return EXIT_FAILURE;
    }

    long int n = atol(argv[1]);
    const char *file_in = argv[2];
    const char *file_out = argv[3];
    int num_threads = (argc == 5) ? atoi(argv[4]) : 4;

    if (n <= 0) {
        fprintf(stderr, "N deve ser > 0\n");
        return EXIT_FAILURE;
    }

    if (num_threads <= 0) {
        num_threads = 4;
    }

    int *v = load_int_vector(file_in, n);
    if (!v) {
        fprintf(stderr, "Erro ao carregar vetor de %s\n", file_in);
        return EXIT_FAILURE;
    }

    double start = omp_get_wtime();
    quicksort_parallel_impl(v, n, num_threads);
    double end = omp_get_wtime();

    if (save_int_vector(v, n, file_out) != 0) {
        fprintf(stderr, "Erro ao salvar vetor ordenado em %s\n", file_out);
    }

    printf("Threads: %d\n", num_threads);
    printf("Tempo paralelo (s): %f\n", end - start);

    free(v);
    return EXIT_SUCCESS;
}
